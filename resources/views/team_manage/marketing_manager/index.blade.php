        ...
        <div class="col-12 col-sm-12">
            <div class="card ">
                <div class="card-header d-flex align-items-center justify-content-between">
                    <h3 class="card-title mb-0">Marketing Managers</h3>
                   @php
                        $permission = new \App\Models\Permission();
                        $create_check = $permission->checkPermission('marketingmanagers.create');
                    @endphp
                    @if ($create_check == 1)
                    <a class="btn-primary add_master_btn" href="{{ route('marketing_manager_create') }}"><span> <i
                                class="fe fe-plus"></i></span>Add New</a>
                    @endif
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="marketing_manager_lists" class="table table-bordered text-nowrap mb-0">
                            <thead class="border-top">
                                <tr>
                                    <th class="bg-transparent border-bottom-0 w-5" style="width: 31.9688px;">S.no</th>
                                    <th class="bg-transparent border-bottom-0" style="width: 185.922px;">Marketing Manager ID</th>
                            ...
                            <tbody>
                                ...
                                @foreach ($marketing_managers as $marketing_manager)
                                    <tr class="border-bottom">
                                        ...
                                        <td>{{ $marketing_manager->reference_code }}</td>
                                        <td>
                                            {{ $marketing_manager->name }}
                                        </td>
                                        <td>
                                            {{ $marketing_manager->mobile_no }}
                                        </td>
                                        <td>
                                            {{ date('d-m-Y', strtotime($marketing_manager->dob)) }}
                                        </td>
                                        ...
                                        <td class="">
                                             @php
                                                $permission = new \App\Models\Permission();
                                                $edit_check = $permission->checkPermission('marketingmanagers.edit');
                                            @endphp
                                            @if($edit_check == 1)
                                            <a class="btn-primary border-0 me-1"
                                                href="{{ url('/') }}/marketing-manager/{{ $marketing_manager->id }}/edit"
                                            ...
                                            @php
                                                $permission = new \App\Models\Permission();
                                                $delete_check = $permission->checkPermission('marketingmanagers.delete');
                                            @endphp
                                            @if($delete_check == 1)
                                            <button class="btn-danger border-0" data-bs-toggle="tooltip"
                                                data-bs-original-title="Delete" style="border-radius: 5px;"><i><svg
                                                        ...
                                                        onclick="deleteOrder('{{ $marketing_manager->id }}')">
                                                        <path d="M0 0h24v24H0V0z" fill="none" />
                                                        <path
...
@section('scripts')
    <script>
        $(document).ready(function() {
            var table = $('#marketing_manager_lists').DataTable();
        });
        ...
        function deleteOrder(id) {
            swal({
                    title: "Are you sure?",
                    text: "Confirm to delete this Marketing Manager?",
                    type: "warning",
                ...
                function(isConfirm) {
                    if (isConfirm) {
                        var redirect = $('meta[name="base_url"]').attr('content') + '/marketing-manager-lists';
                        var token = $('meta[name="csrf-token"]').attr("content");
                        var formData = new FormData();
                        formData.append("_token", "{{ csrf_token() }}");
                        formData.append("id", id);
                        $.ajax({
                            url: "{{ route('marketing_manager_destroy', '') }}" + "/" + id,
                            data: formData,
                            type: 'DELETE',
                            contentType: false,
                            processData: false,
                            ...
                            success: function(res) {
                                if (res) {
                                    swal("Deleted!", "Marketing Manager has been deleted.", "success");
                                    window.location.href = redirect;

                                } else {
                                    swal("Marketing Manager Delete Failed", "Please try again. :)",
                                        "error");
                                }
                            }
                        });
