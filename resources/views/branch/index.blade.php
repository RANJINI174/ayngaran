...
@section('content')
    ...
    {{-- <div class="page-header">
        ...
        <div class="ms-auto pageheader-btn">
            <a class="modal-effect btn btn-primary btn-icon text-white me-2" data-bs-effect="effect-fall"
                data-bs-toggle="modal" href="#Add_branchModel"><span>
    ...
    <!-- PAGE-HEADER END -->
    {{-- Add Branch --}}
    <div class="modal fade" id="Add_branchModel">
        <div class="modal-dialog modal-dialog-centered text-center" role="document">
            <div class="modal-content modal-content-demo">
                <div class="modal-header">
                    <h6 class="modal-title">Add Branch</h6><button aria-label="Close" class="btn-close"
                        data-bs-dismiss="modal"><span aria-hidden="true">&times;</span></button>
                </div>
                <div class="modal-body">
                    <form id="Add_branchForm" autocomplete="off">
                        @csrf
                        @method('POST')
                        <div class="form-group">
                            <input type="hidden" name="url" id="url" value="{{ route('branch.store') }}">
                            <input type="text" class="form-control" id="branch_name" name="branch_name"
                                placeholder="Branch Name">
                            <div class="text-start text-danger branch_name"></div>
                        </div>
                        <div class="form-group">
    ...
    {{-- Edit Branch --}}
    <div class="modal fade" id="Edit_branchModel">
        <div class="modal-dialog modal-dialog-centered text-center" role="document">
            <div class="modal-content modal-content-demo">
                <div class="modal-header">
                    <h6 class="modal-title">Edit Branch</h6><button aria-label="Close" class="btn-close"
                        data-bs-dismiss="modal"><span aria-hidden="true">&times;</span></button>
                </div>
                <div class="modal-body">
                    <form id="Edit_branchForm" autocomplete="off">
                        @csrf
                        @method('PUT')
                        <div class="form-group">
                            <input type="hidden" id="branch_id">
                            <input type="text" class="form-control" id="edit_branch" name="edit_branch"
                                placeholder="Branch Name">
                            <div class="text-start text-danger edit_branch"></div>
                        </div>
                        <div class="form-group">
            ...
            <div class="card ">
                <div class="card-header  d-flex align-items-center justify-content-between">
                    @php
                        ...
                        $check = $permission->checkPermission('branch.create');
                    @endphp
                    <h3 class="card-title mb-0">Branch</h3>
                    @if ($check == 1)
                        <button class="add_master_btn" data-bs-effect="effect-fall" data-bs-toggle="modal"
                            href="#Add_branchModel"><span>
                                <i class="fe fe-plus"></i>
                            </span> Add New</button>
                    @endif
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="branch_table" class="table table-bordered text-nowrap mb-0">
                                ...
                                @foreach ($branches as $branch)
                                    <tr class="border-bottom">
                                        ...
                                        <td class="">
                                            @php
                                                $permission = new \App\Models\Permission();
                                                $check = $permission->checkPermission('branch.edit');
                                            @endphp
                                            @if ($check == 1)
                                                <button class="bg-primary border-0 me-1" data-bs-effect="effect-fall"
                                                    data-bs-toggle="modal" onclick="return EditBranch({{ $branch->id }})"
                                            ...
                                            @php
                                                $permission = new \App\Models\Permission();
                                                $check = $permission->checkPermission('branch.delete');
                                            @endphp
                                            @if ($check == 1)
...
@section('scripts')
    <script>
        ...
        function deleteOrder(id) {
            swal({
                    title: "Are you sure?",
                    text: "Confirm to delete this Branch?",
                    type: "warning",
                        ...
                        $.ajax({
                            ...
                            success: function(res) {
                                if (res) {
                                    ...
                                    swal("Branch Delete Failed", "Please try again. :)", "error");
                                }
                            }
                        });
