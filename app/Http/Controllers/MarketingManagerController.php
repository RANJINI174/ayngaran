...
class MarketingManagerController extends Controller
{
    ...
    {
         try {
            $marketing_managers = User::where('user_type','marketing-manager')->get()->sortBy('reference_code', SORT_NATURAL);;
    ...
    public function create()
    {
        try {
            ...
            return view('team_manage.marketing_manager.add', compact('count', 'branch', 'relations', 'designation', 'directors','banks'));
        } catch (\Exception $e) {
            return back()->with(['error' => $e->getMessage()])->withInput();
        }
    }
...
curl_close($curl);
    ...
    public function edit($id)
    {
        if (!empty($id)) {
            ...
            $designation = Designation::where('status',1)->get();
            $banks = Bank::where('status',1)->get();
            return view('team_manage.marketing_manager.edit', compact('marketing_manager', 'directors', 'areas', 'state', 'city', 'branch', 
            'relations', 'designation', 'introducer_id', 'introducer_name','banks'));
        }
